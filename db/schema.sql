...
CREATE TABLE IF NOT EXISTS ClassSession (
  ...
  textbook      VARCHAR(100)      COMMENT '진도는 반 공통이다. 학생별로 나누지 않는다',
  rangeText     VARCHAR(100)      COMMENT 'p.42~55 형태',
  content       VARCHAR(500)      COMMENT '오늘 한 것',
  homework      VARCHAR(500)      COMMENT '오늘 과제',
  hiddenAt      DATETIME          COMMENT '숨긴 시각. 지우지 않고 목록에서만 뺀다. 같은 날짜로 다시 열면 되살아난다',
  createdAt     DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
...
CREATE TABLE IF NOT EXISTS ProgressLog (
  ...
  note            VARCHAR(500)          COMMENT '학생 메모 / 그날 이 학생에게 눈에 띈 것. 없으면 비워 둔다',
  createdBy       BIGINT,
  createdAt       DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updatedAt       DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
